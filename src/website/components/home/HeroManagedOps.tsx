    ...
    <div className="relative mx-auto w-full max-w-[420px] aspect-square select-none">
      ...
      {channels.map((c, i) => {
        ...
        return (
          ...
          >
            <motion.div
              className={`-translate-x-1/2 -translate-y-1/2 rounded-2xl border bg-card px-3.5 py-2 text-xs font-semibold shadow-soft transition-colors ${
                isActive
                  ? "border-primary/50 text-primary"
                  : "border-border text-muted-foreground"
              }`}
              animate={
                reduceMotion || !isActive ? { scale: 1 } : { scale: [1, 1.06, 1] }
              }
      ...
      {/* The pod */}
      <motion.div
        className="absolute left-1/2 top-1/2 w-[220px] -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-primary/25 bg-card/95 p-4 shadow-strong backdrop-blur-sm"
      ...
      <motion.div
        key={`in-${current.task}`}
        className="absolute left-1/2 top-[8%] -translate-x-1/2 rounded-pill border border-border bg-card px-3 py-1.5 text-[11px] font-medium text-muted-foreground shadow-soft"
      ...
      <motion.div
        ...
        className="absolute bottom-[9%] left-1/2 -translate-x-1/2 rounded-pill border border-primary/35 bg-primary/10 px-3.5 py-1.5 text-[11px] font-semibold text-primary shadow-soft"
